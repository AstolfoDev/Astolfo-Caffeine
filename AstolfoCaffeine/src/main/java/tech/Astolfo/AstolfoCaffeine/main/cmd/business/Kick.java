...
public class Kick extends Command {
  ...
  protected void execute(CommandEvent e) {
    ...
    if (comp == null) {
      e.reply(new Logging().error(e.getSelfUser(), "**heY!!** ur not even in a company... u canz kick anyone from it..."));
      return;
    }
    ...
    if (target == null) {
      e.reply(new Logging().error(e.getSelfUser(), "ayfhuwijgg dassss not a valid uzzzerrrr!!11 ;("));
      return;
    }

    BasicDBObject filter2 = new BasicDBObject("members", new BasicDBObject("$in", Arrays.asList(target.getIdLong())))
