    ...
    protected void execute (CommandEvent e) {
        try {
            ...
            if (args.length < 1) {
                ...
                msg.getChannel().sendMessage(new Logging().error(msg.getJDA().getSelfUser(), "sozzzzz! You entered too many arguwuments...\n*(Ex: "+System.getenv("PREFIX")+"pay <@682220266901733381> 10)*")).queue();
                return;
            }
            if (mentions.get(0).isBot()) {
                ...
                msg.getChannel().sendMessage(new Logging().error(msg.getJDA().getSelfUser(), "Sowwy! You fOwOgot to put the amount you want to send...\n*(Ex: "+System.getenv("PREFIX")+"pay <@682220266901733381> 10)*")).queue();;
                return;
            }
            ...
            if (amt < 1) {
              msg.getChannel().sendMessage(new Logging().error(msg.getJDA().getSelfUser(), "u canz send less than 1 credit!!!")).queue();
              return;
            }

            if (amt > Double.parseDouble(doc.get("credits").toString())) {
              msg.getChannel().sendMessage(new Logging().error(msg.getJDA().getSelfUser(), "OooooOwOh nuuuu! u dont hazzz enough cwedits 4 dat transfer!!!")).queue();
              return;
            }
