...
public class Create extends Command {
    ...
    protected void execute(CommandEvent e) {
        ...
        if (comp != null) {
            e.reply(new Logging().error(e.getSelfUser(), "oi! ur already in a business... leave it before creating a new one!!"));
            return;
        }
        ...
        waiter.waitForEvent(
                ...
                e2 -> {
                    if (e2.getMessage().getContentRaw().length() > 16) {
                        e.reply(new Logging().error(e.getSelfUser(), "**SoowWy!** the name can only be 16 characters long or less..."));
                        return;
                    }
                                ...
                                waiter.waitForEvent(
                                        ...
                                        e4 -> {
                                            if (e4.getMessage().getAttachments().size() > 1) {
                                                e.reply(new Logging().error(e.getSelfUser(), "too many attatchments sent! using default logo!"));
                                                ...
                                                if (validExt.contains(ext.toLowerCase())) {
                                                    ...
                                                    e.reply(new Logging().error(e.getSelfUser(), "invalid image type! using default logo!"));
                                                    final String logo = "https://media.discordapp.net/attachments/738514936338055178/745569159550730280/flat750x075f-pad750x1000f8f8f8.u3.jpg";
                                                    finalCalc(e4.getMessage(), name, description, logo);
                                                }
                                            } else {
                                                e.reply(new Logging().error(e.getSelfUser(), "no image sent! using default logo!"));
                                                final String logo = "https://media.discordapp.net/attachments/738514936338055178/745569159550730280/flat750x075f-pad750x1000f8f8f8.u3.jpg";
                                                finalCalc(e4.getMessage(), name, description, logo);
                                            }
                                        },
