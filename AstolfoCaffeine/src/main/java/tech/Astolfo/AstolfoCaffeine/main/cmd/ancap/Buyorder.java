...
public class Buyorder extends Command {
    ...
    public Buyorder() {
        super.name = "buyorder";
        super.aliases = new String[]{"bo","buy"};
        super.help = "purchase shares from the market";
    ...
    protected void execute (CommandEvent e) {
        ...
        try {
            ...
            if (args.length < 1 || args[0] == "") {
                msg.getChannel().sendMessage(new Logging().error(msg.getJDA().getSelfUser(), "nuuuuu! u fOwOgot to write which stock u wanz to buy...\n*(Ex. "+System.getenv("PREFIX")+"buyorder ASTF 100)*")).queue();
                return;
            } else if (args.length < 2) {
                msg.getChannel().sendMessage(new Logging().error(msg.getJDA().getSelfUser(), "ahhhhH!!! u forgot to specifyyyy how many shares u wanna buyyYy...\n*(Ex. "+System.getenv("PREFIX")+"buyorder ASTF 100)*")).queue();
                return;
            }
            ...
            switch (stock) {
                ...
                case "AMZN":
                    ...
                    if (price > doc.getDouble("credits")) {
                      msg.getChannel().sendMessage(new Logging().error(msg.getJDA().getSelfUser(), "ahHHHHHHhhhh u need **"+price+"** "+cr+" to afford **x"+amt+"** shares!!!11!!")).queue();
                      return;
                    }
                    ...
                    embed = eb
                        .setDescription("Buy order successfully placed!")
            ...
            new Logging().error(msg.getJDA().getSelfUser(), "OooooOoh no! u gave me and invalid numbwer ;(\n*(Hint: make sure it's a whole number!)*");
        }
    }
}