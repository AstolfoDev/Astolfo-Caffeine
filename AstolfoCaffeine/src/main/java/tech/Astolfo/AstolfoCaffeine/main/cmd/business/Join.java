...
public class Join extends Command {
  ...
  protected void execute(CommandEvent e) {
    ...
    BasicDBObject filter = new BasicDBObject("members", new BasicDBObject("$in",Arrays.asList(e.getMessage().getAuthor().getIdLong())));
    Document comp1 = App.company.find(filter).first();

    if (comp1 != null) {
      e.reply(new Logging().error(e.getSelfUser(), "EEEERMMMMM ur sorta in a company already... leave it first before joining a new one!!! ;P"));
      return;
    }

    String name = e.getArgs();
    if (name == "") {
      e.reply(new Logging().error(e.getSelfUser(), "nah nah nah u cant just put no args, mate. pls lmk which comp u wanna jooooin thx ;3"));
      return;
    }
    ...
    if (doc == null) {
      e.reply(new Logging().error(e.getSelfUser(), "omgomogmogmgomgomg literally no one from `"+name+"` invited u... :expressionless:"));
      return;
    }
